...
API_KEY_1 = 'AIzaSyARhR9PCMuiNXdGTU9yA2sfA9EAJ7ZqMNE'

API_KEY_2 = 'AIzaSyCtnIoQfXiZSb0ske3CBUCGuvjznNVZaTI' # NILOTPAL.KUMAR@AAJTAK.COM API_KEY 1

API_KEY_3 = 'AIzaSyDcwyGezZYTMfFtE-10UUsxi9K51Nbb5uM' # NILOTPAL.KUMAR@AAJTAK.COM API_KEY 2
...
def get_current_api_key():
    """Returns the correct API key based on the current time."""
    # Get current time in IST (India Standard Time)
    ist = pytz.timezone('Asia/Kolkata')
    current_time = datetime.now(ist).time()

    # Define the time intervals for each API key
    if current_time >= datetime.strptime("07:00:00", "%H:%M:%S").time() and current_time < datetime.strptime("13:00:00", "%H:%M:%S").time():
        return API_KEY_1  # 7:00 AM - 12:59 PM
    elif current_time >= datetime.strptime("13:00:00", "%H:%M:%S").time() and current_time < datetime.strptime("19:00:00", "%H:%M:%S").time():
        return API_KEY_2  # 1:00 PM - 6:59 PM
    elif current_time >= datetime.strptime("19:00:00", "%H:%M:%S").time() and current_time < datetime.strptime("01:00:00", "%H:%M:%S").time():
        return API_KEY_3  # 7:00 PM - 12:59 AM
    else:
        return API_KEY_1  # 1:00 AM - 6:59 AM (reverts back to API Key 1)
...
if __name__ == "__main__":
    fetch_and_compare_live_videos(channel_ids, API_KEY_1)
