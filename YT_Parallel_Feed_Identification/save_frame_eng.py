...
SAVE_DIR = f'English_captured_frames_{timestap}'
...
# Function to save frame to a file
def save_frame(frame, channel_id, video_id, timestamp, frame_type):
    """Save the frame as an image file with a timestamp and channel_id."""
    filename = f"{channel_id}_{video_id}_{frame_type}_{timestamp}.jpg"
...
def get_timestamp():
    """Return the current timestamp in the format YYYYMMDD_HHMMSS."""
...
# Capture a frame from a live video using yt-dlp and OpenCV
...
def capture_frames_parallel(primary_video_id, compare_video_id, channel_id):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        # # Save both frames with proper file names if they are successfully captured
        # if primary_frame is not None:
        #     save_frame(primary_frame, channel_id, primary_video_id, timestamp, 'primary')
        # if compare_frame is not None:
        #     save_frame(compare_frame, channel_id, compare_video_id, timestamp, 'compare')
...
def fetch_and_compare_live_videos(channel_ids, api_key):
    ...
    if live_videos_with_primary:
        ...
        df['MSE'] = None
        df['Is_Parallel'] = None
        df['Exception'] = None  # For logging any errors
        ...
        for index, row in df.iterrows():
            ...
            if video_id != primary_video_id:
                ...
                try:
                    ...
                    primary_frame, compare_frame = capture_frames_parallel(primary_video_id, video_id, channel_id)
                    ...
                    if primary_frame is not None and compare_frame is not None:
                       ...
                       # Save both frames with proper file names if they are successfully captured
                    timestamp = get_timestamp()  # Get the current timestamp for file names
                    if primary_frame is not None:
                        save_frame(primary_frame, channel_id, primary_video_id, timestamp, 'primary')
                    if compare_frame is not None:
                       save_frame(compare_frame, channel_id, video_id, timestamp, 'compare')

                except Exception as e:
                    df.at[index, 'Exception'] = str(e)
                    print(f"Error comparing video {video_id} from {row['Channel_Name']}: {str(e)}")

        # Save the results to an Excel file
        df.to_excel(f'English_yt_dlp_live_videos_with_primary_and_comparison_{timestamp}.xlsx', index=False)
        print("Saved live video details with primary and comparison to 'English_yt_dlp_live_videos_with_primary_and_comparison_{timestamp}.xlsx'.")
    else:
